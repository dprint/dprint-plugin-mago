...
function buildReviewPrompt(options: AiFixOptions, diff: string): string {
  return [
    ...
    ``,
    `Check specifically:`,
    `- The mapping in \`build_format_settings\` matches mago-formatter's current \`FormatSettings\` (no removed/renamed field is still referenced, no wrong field is set).`,
    `- Any newly exposed config option is wired through ALL layers: configuration.rs, resolve_config.rs, format_text.rs, deployment/schema.json, and README.md. A partial addition is a blocking issue.`,
